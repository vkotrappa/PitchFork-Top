...
async function sendAdminEmailViaGmail(
  toEmail: string,
  senderName: string,
  companyName: string,
  messageTitle: string,
  ...
  console.log('Initializing SMTP client for admin email...');
...
This message was sent via Pitch Fork platform.`;
  ...
  try {
    ...
    console.log('Connecting to Gmail SMTP for admin email...');
    ...
    console.error('SMTP Error for admin email:', error);
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { companyName, messageTitle, messageDetail }: AdminEmailRequest = await req.json();

    if (!messageTitle || !messageDetail) {
      throw new Error('Message title and detail are required');
    }

    console.log('Sending admin email from:', senderName);
    console.log('Company:', companyName);
    console.log('Title:', messageTitle);

    // Send email to admin
    await sendAdminEmailViaGmail(
      'vkotrappa@gmail.com',
      senderName,
      companyName,
      messageTitle,
